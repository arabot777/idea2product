...
const FluxDevSchema = z.object({
  ...
  guidance_scale: z.number().min(1).max(10).default(3.5).describe('The CFG (Classifier Free Guidance) scale is a measure of how close you want the model to stick to your prompt when looking for a related image to show you.'),
  ...
  seed: z.number().int().default(-1).describe('The same seed and the same prompt given to the same version of the model will output the same image every time.'),
...
export class FluxDevRequest extends BaseRequest<typeof FluxDevSchema> {
  ...
  protected data: z.infer<typeof FluxDevSchema>;
