...
const WanFlf2vSchema = z.object({
  ...
  resolution: z.enum(["720p", "480p"]).optional().default("720p").describe("Resolution of the generated video (480p or 720p). 480p is 0.5 billing units, and 720p is 1 billing unit."),
  num_inference_steps: z.number().int().min(0).max(100).optional().default(30).describe("Number of inference steps for sampling. Higher values give better quality but take longer. "),
  guide_scale: z.number().min(1.01).max(30).optional().default(5).describe("Classifier-free guidance scale. Higher values give better adherence to the prompt but may decrease quality. "),
  ...
  aspect_ratio: z.enum(["auto", "1:1", "9:16", "16:9"]).optional().default("auto").describe("Aspect ratio of the generated video. If 'auto', the aspect ratio will be determined automatically based on the input image.")
});
