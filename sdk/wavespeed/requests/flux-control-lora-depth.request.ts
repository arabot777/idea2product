...
const FluxControlLoraDepthSchema = z.object({
  ...
  seed: z.number().int().min(-1).max(9999999999).optional().default(0).describe('The same seed and the same prompt given to the same version of the model will output the same image every time.'),
  ...
  guidance_scale: z.number().min(1).max(30).step(0.1).optional().default(3.5).describe('The CFG (Classifier Free Guidance) scale is a measure of how close you want the model to stick to your prompt when looking for a related image to show you'),
...
export class FluxControlLoraDepthRequest extends BaseRequest<typeof FluxControlLoraDepthSchema> {
  ...
  protected data: z.infer<typeof FluxControlLoraDepthSchema>;
