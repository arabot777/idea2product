...
const FluxKontextProTextToImageSchema = z.object({
  ...
  aspect_ratio: z.enum(["21:9", "16:9", "4:3", "3:2", "1:1", "2:3", "3:4", "9:16", "9:21"]).optional().default("1:1").describe('The aspect ratio of the generated image.'),
  guidance_scale: z.number().min(1).max(10).optional().default(3.5).describe('The CFG (Classifier Free Guidance) scale is a measure of how close you want the model to stick to your prompt when looking for a related image to show you.'),
  num_images: z.number().int().min(1).max(4).optional().default(1).describe('The number of images to generate.'),
  safety_tolerance: z.enum(["1", "2", "3", "4", "5"]).optional().default("2").describe('The safety tolerance level for the generated image. 1 being the most strict and 5 being the most permissive.'),
...
export class FluxKontextProTextToImageRequest extends BaseRequest<typeof FluxKontextProTextToImageSchema> {
  ...
  protected data: z.infer<typeof FluxKontextProTextToImageSchema>;
