    ...
    <nav className="fixed top-0 w-full z-50 bg-slate-900/80 backdrop-blur-xl border-b border-slate-700/50">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center space-x-8">
          ...
          <Link href="/subscribe-plan" className="text-slate-300 hover:text-white transition-colors duration-200 font-medium">
            {t("subscribePlan")}
          </Link>
        </div>
        ...
        <div className="flex items-center space-x-4">
          {/* Mobile Menu Button */}
          <Button variant="ghost" size="sm" className="md:hidden text-white hover:text-white" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {user && user.id ? (
            <DropdownMenu modal={false}>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-10 w-10 rounded-full ring-2 ring-slate-700 hover:ring-blue-500 transition-all duration-200">
              ...
              <DropdownMenuContent className="w-56 bg-slate-800/95 backdrop-blur-xl border-slate-700 overflow-hidden" align="end" forceMount>
                ...
                <DropdownMenuItem asChild className="hover:bg-slate-700/50">
                  <button onClick={handleSignOut} className="w-full flex items-center text-red-400 hover:text-red-400">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-slate-800/95 backdrop-blur-xl border-t border-slate-700/50">
          <div className="px-6 py-4 space-y-3">
            ...
            <Link href="/history" className="block text-slate-300 hover:text-white transition-colors duration-200 font-medium">
              {t("history")}
            </Link>
            <Link href="/subscribe" className="block text-slate-300 hover:text-white transition-colors duration-200 font-medium">
              {t("subscribe")}
            </Link>
          </div>
        </div>
